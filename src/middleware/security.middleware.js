...
const securityMiddleware = async (req, res, next) => {
  try {
    ...
    const client = aj.withRule(slidingWindow({mode: 'LIVE', interval: '1m', max: limit, name: role+'RateLimit'}));
    ...
    if(decision.isDenied() && decision.reason.isBot()){
      ...
      return res.status(403).json({error: 'Bot request blocked', message: 'Automated requests are not allowed'});
    }

    if(decision.isDenied() && decision.reason.isShield()){
      logger.warn('Shield request blocked',{ip: req.ip, userAgent: req.get('User-Agent'), path: req.path, method: req.method});

      return res.status(403).json({error: 'Shield request blocked', message: 'Request blocked by security policy'});
    }
